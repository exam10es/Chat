...
if (!enforce_rate_limit('api_specializations', 100, 60)) {
...
if ($method === 'PUT') {
    $id = (int) ($_GET['id'] ?? 0);
    $stmt = db()->prepare('UPDATE specializations SET name = :name, description = :description, display_order = :display_order, is_active = :is_active WHERE id = :id');
    $stmt->execute([
        ':name' => clean_input($input['name'] ?? ''),
        ':description' => clean_input($input['description'] ?? ''),
        ':display_order' => (int) ($input['display_order'] ?? 0),
...
if ($method === 'DELETE') {
    $id = (int) ($_GET['id'] ?? 0);
    $stmt = db()->prepare('DELETE FROM specializations WHERE id = :id');
    $stmt->execute([':id' => $id]);
