...
if (!enforce_rate_limit('api_chapters', 100, 60)) {
...
if ($method === 'GET') {
    ...
    $stmt = db()->query('SELECT * FROM chapters ORDER BY display_order, id');
    Response::json(true, 'تم التحميل', $stmt->fetchAll());
}
...
if ($method === 'PUT') {
    $id = (int) ($_GET['id'] ?? 0);
    $stmt = db()->prepare('UPDATE chapters SET subject_id = :subject_id, name = :name, description = :description, display_order = :display_order, is_active = :is_active WHERE id = :id');
    $stmt->execute([
        ':subject_id' => (int) ($input['subject_id'] ?? 0),
        ':name' => clean_input($input['name'] ?? ''),
        ':description' => clean_input($input['description'] ?? ''),
