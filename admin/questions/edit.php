    ...
    <form method="post" class="card">
        ...
        <select name="chapter_id" required>
            <?php foreach ($chapters as $chapter): ?>
                <option value="<?= (int) $chapter['id']; ?>" <?= $chapter['id'] == $question['chapter_id'] ? 'selected' : ''; ?>>
                    <?= htmlspecialchars($chapter['name'], ENT_QUOTES, 'UTF-8'); ?>
                </option>
            <?php endforeach; ?>
        </select>
        <label>نص السؤال</label>
        <textarea name="question_text" required><?= htmlspecialchars($question['question_text'], ENT_QUOTES, 'UTF-8'); ?></textarea>
        ...
        <input type="text" name="option_a" value="<?= htmlspecialchars($question['option_a'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
        <label>الخيار B</label>
        <input type="text" name="option_b" value="<?= htmlspecialchars($question['option_b'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
        <label>الخيار C</label>
        <input type="text" name="option_c" value="<?= htmlspecialchars($question['option_c'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
        <label>الخيار D</label>
        <input type="text" name="option_d" value="<?= htmlspecialchars($question['option_d'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
        <label>الإجابة الصحيحة</label>
        <input type="text" name="correct_answer" value="<?= htmlspecialchars($question['correct_answer'], ENT_QUOTES, 'UTF-8'); ?>" required>
        <label>الصعوبة</label>
        ...
        <label>الدرجة</label>
        <input type="number" name="points" value="<?= (int) $question['points']; ?>">
        <label>ترتيب العرض</label>
        <input type="number" name="display_order" value="<?= (int) $question['display_order']; ?>">
        <label>
            <input type="checkbox" name="is_active" <?= $question['is_active'] ? 'checked' : ''; ?>> نشط
        </label>
